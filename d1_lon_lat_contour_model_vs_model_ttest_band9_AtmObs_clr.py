...
#def lon_lat_contour_model_vs_model(varnmtoa,season,scale_ctl,scale_exp,table):
# data path
...
#---
varnmtoa="FSSD04"  #np.array(["FLNS","SOLIN","LHFLX","SHFLX"])
varnmtoa_sub="FSSUCLR04"  #np.array(["FLNS","SOLIN","LHFLX","SHFLX"])
varnmsfc="FSSDCLRS04"  #np.array(["FLNS","SOLIN","LHFLX","SHFLX"])
varnmsfc_sub="FSSUCLRS04"  #np.array(["FLNS","SOLIN","LHFLX","SHFLX"])
season="ANN"
...
for iy in range(0,years.size):
   ...
   means_yby_ctl[iy,:,:]=file_ctl_2.variables[varnmtoa][0,:,:]-file_ctl.variables[varnmtoa_sub][0,:,:]-\
                         (file_ctl.variables[varnmsfc][0,:,:]-file_ctl.variables[varnmsfc_sub][0,:,:])
   means_yby_exp[iy,:,:]=file_exp_2.variables[varnmtoa][0,:,:]-file_exp.variables[varnmtoa_sub][0,:,:]-\
                         (file_exp.variables[varnmsfc][0,:,:]-file_exp.variables[varnmsfc_sub][0,:,:])
...
# make plot
#parameters=get_parameters(varnmtoa,season)
...
diff_levs=np.linspace(-0.6,0.6,13) #[-50, -40, -30, -20, -10, -5, 5, 10, 20, 30, 40, 50]
colormap="PiYG_r"
colormap_diff="bwr"
...
for i in range(0,1):
   #1. first plot
    ...
    # title
    ax.set_title("Diff in Atm Abs CLR (TSIS - CESM2, Band 0.44-0.63)",loc="center",fontdict=plotSideTitle)
...
#fig.suptitle(varnmtoa, x=0.5, y=0.96, fontsize=14)
#save figure as file
#if os.environ["fig_save"]=="True":
#    fname="d1_lon_lat_contour_"+varnmtoa+"_"+season+"."+os.environ["fig_suffix"]
#    plt.savefig(os.environ["OUTDIR"]+"/figures/"+fname)
#plt.savefig(figure_name+".png")
#if os.environ["fig_show"]=="True":
#    plt.show()
#plt.savefig("./figures/diff_atm_abs_clr_band_0.44-0.63.eps")
plt.show()
plt.close()
