...
#---
varnmtoa="FSNTOAC"  #np.array(["FLNS","SOLIN","LHFLX","SHFLX"])
varnmsfc="FSNSC"  #np.array(["FLNS","SOLIN","LHFLX","SHFLX"])
season="ANN"
...
for iy in range(0,years.size):
   ...
   means_yby_ctl[iy,:,:]=file_ctl.variables[varnmtoa][0,:,:]-file_ctl.variables[varnmsfc][0,:,:]
   means_yby_exp[iy,:,:]=file_exp.variables[varnmtoa][0,:,:]-file_ctl.variables[varnmsfc][0,:,:]
...
for i in range(0,1):
   #1. first plot
    ...
    # title
    ax.set_title("Diff in Atm SW Abs (TSIS - CESM2)",loc="center",fontdict=plotSideTitle)
