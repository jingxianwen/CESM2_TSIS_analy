...
#------------------------
out_path='./results_ocn_lnd_cbaseNoPrec/'
...
flist=sorted(glob.glob(data_path+"*2001-*.nc"))
...
# sample 
   ...
   mixphs_flag = (ctot_ice > 10.) * (ctot_liq > 10.)   # mixed phase clouds only
   #print(scop[0,:,0,20,0])
   #print(scop[0,:,1,20,0])
   #print(landfrac[0,:,0])
   #print(ctot_liq[0,:,0])
...
fout=open(out_path+'cnt_cld_lnd_NH_30-60.txt','w')
for il in range(num_tcbin):
      fout.write(str(cnt_samp_N2[0,il,:])+'\n')
fout.close()

fout=open(out_path+'cnt_cld_lnd_NH_60-90.txt','w')
for il in range(num_tcbin):
      fout.write(str(cnt_samp_N3[0,il,:])+'\n')
fout.close()

fout=open(out_path+'cnt_cld_lnd_SH_0-30.txt','w')
for il in range(num_tcbin):
      fout.write(str(cnt_samp_S1[0,il,:])+'\n')
fout.close()

fout=open(out_path+'cnt_cld_lnd_SH_30-60.txt','w')
for il in range(num_tcbin):
      fout.write(str(cnt_samp_S2[0,il,:])+'\n')
fout.close()

fout=open(out_path+'cnt_cld_lnd_SH_60-90.txt','w')
for il in range(num_tcbin):
      fout.write(str(cnt_samp_S3[0,il,:])+'\n')
fout.close()

#--------
fout=open(out_path+'cnt_cld_ocn_NH_0-30.txt','w')
for il in range(num_tcbin):
      fout.write(str(cnt_samp_N1[1,il,:])+'\n')
fout.close()

fout=open(out_path+'cnt_cld_ocn_NH_30-60.txt','w')
for il in range(num_tcbin):
      fout.write(str(cnt_samp_N2[1,il,:])+'\n')
fout.close()

fout=open(out_path+'cnt_cld_ocn_NH_60-90.txt','w')
for il in range(num_tcbin):
      fout.write(str(cnt_samp_N3[1,il,:])+'\n')
fout.close()

fout=open(out_path+'cnt_cld_ocn_SH_0-30.txt','w')
for il in range(num_tcbin):
      fout.write(str(cnt_samp_S1[1,il,:])+'\n')
fout.close()

fout=open(out_path+'cnt_cld_ocn_SH_30-60.txt','w')
for il in range(num_tcbin):
      fout.write(str(cnt_samp_S2[1,il,:])+'\n')
fout.close()

fout=open(out_path+'cnt_cld_ocn_SH_60-90.txt','w')
for il in range(num_tcbin):
