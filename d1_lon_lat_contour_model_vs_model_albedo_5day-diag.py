...
#def lon_lat_contour_model_vs_model(varnm_vis,season,scale_ctl,scale_exp,table):
# data path
...
#---
varnm_vis="ASDIF"  #np.array(["FLNS","SOLIN","LHFLX","SHFLX"])
varnm_nir="ALDIF"  #np.array(["FLNS","SOLIN","LHFLX","SHFLX"])
season="ANN"
...
means_vis=np.zeros((nlat,nlon)) #year by year mean for each variable
means_nir=np.zeros((nlat,nlon)) #year by year mean for each variable
...
means_vis[:,:]=file_ctl.variables[varnm_vis][0,:,:] #-file_ctl.variables[varnm_nir][0,:,:]
means_nir[:,:]=file_ctl.variables[varnm_nir][0,:,:] #-file_ctl.variables[varnm_nir][0,:,:]
#means_exp[:,:]=file_exp.variables[varnm_vis][0,:,:] #-file_ctl.variables[varnm_nir][0,:,:]

diffs=means_nir-means_vis
print(diffs[45,:])
tmp=np.zeros((nlat,nlon))
#stats_all=np.zeros((3,3))
#stats_now=np.zeros((3))
tmp[:,:]=means_vis[:,:]
stats_vis=get_area_mean_min_max(tmp,lat[:])
tmp[:,:]=means_nir[:,:]
stats_nir=get_area_mean_min_max(tmp,lat[:])
tmp[:,:]=diffs[:,:]
...
# add cyclic
#dtctl=add_cyclic_point(means_vis[:,:])
...
# make plot
#parameters=get_parameters(varnm_vis,season)
...
diff_levs=np.linspace(-0.12,0.12,9) #[-50, -40, -30, -20, -10, -5, 5, 10, 20, 30, 40, 50]
#diff_levs=np.linspace(0.0,2.0,11) #[-50, -40, -30, -20, -10, -5, 5, 10, 20, 30, 40, 50]
colormap="PiYG_r"
...
for i in range(0,1):
   #1. first plot
    ...
    # title
    ax.set_title("Diff in Diffuse Albedo (NIR - VIS)",loc="center",fontdict=plotSideTitle)
    ...
    fig.text(panel[0][0] + 0.705, panel[0][1] + 0.645,
             "Mean="+"%.2f"%(stats_now[0]), ha='left', fontsize=11)
             #"Max\nMean\nMin", ha='left', fontdict=plotText)
    #fig.text(panel[0][0] + 0.76, panel[0][1] + 0.61, "%.2f\n%.2f\n%.2f" %
    #fig.text(panel[0][0] + 0.80, panel[0][1] + 0.61, "%.2f" %
    #        (stats_now[0]), ha='left', fontsize=12)
...
#fig.suptitle(varnm_vis, x=0.5, y=0.96, fontsize=14)
#save figure as file
#if os.environ["fig_save"]=="True":
#    fname="d1_lon_lat_contour_"+varnm_vis+"_"+season+"."+os.environ["fig_suffix"]
#    plt.savefig(os.environ["OUTDIR"]+"/figures/"+fname)
#plt.savefig(figure_name+".png")
#if os.environ["fig_show"]=="True":
#    plt.show()
#plt.savefig("./figures/diff_atm_abs_sw.eps")
plt.show()
plt.close()
