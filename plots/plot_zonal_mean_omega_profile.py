...
var_group_todo=1
# variable group 1:
varnms=np.array(["OMEGA"])
#varnms=np.array(["FSNTOA","FSNS","TS"])
var_long_name="OMEGA"
figure_name="OMEGA_zonal_DJF"
units="mb/day"
...
for iy in range(0,years.size): 
    # open data file
    fctl=fpath_ctl+ctl_pref+"_climo_DJF.nc"
    fexp=fpath_exp+exp_pref+"_climo_DJF.nc"
...
#siglev=0.05
means_ctl=np.mean(means_yby_ctl,axis=0)*24.*3600.*0.01
means_exp=np.mean(means_yby_exp,axis=0)*24.*3600.*0.01
