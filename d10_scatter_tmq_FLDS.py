...
#---
varnm_1="TMQ"  #np.array(["FLNS","SOLIN","LHFLX","SHFLX"])
varnm_2="FLDS"     #np.array(["FLNS","SOLIN","LHFLX","SHFLX"])
season="ANN"
...
for iy in range(0,years.size):
   ...
   means_yby_ctl_rad[iy,:,:]=file_ctl.variables[varnm_2][0,:,:] #+\
                             #file_ctl.variables["FLNS"][0,:,:]-\
                             #file_ctl.variables["FLNSC"][0,:,:]
                         
   means_yby_exp_rad[iy,:,:]=file_exp.variables[varnm_2][0,:,:] #+\
                             #file_exp.variables["FLNS"][0,:,:] -\
                             #file_exp.variables["FLNSC"][0,:,:]
...
i_50S=np.max(np.where(lat[:]<-60))+1
i_50N=np.max(np.where(lat[:]<60))+1
i_70S=np.max(np.where(lat[:]<-85))+1
i_70N=np.max(np.where(lat[:]<85))+1
nlat=len(lat[:])
...
fig.text(0.5,0.05,r"Diff in Water Vapor (kgm$^2$)",fontsize=14,va='center',ha='center')
fig.text(0.03,0.5,r"Diff in SFC Down LW (Wm$^-$$^2$)",fontsize=14,va='center',ha='center',rotation='vertical')
#plt.savefig("./figures/scat_icefrac_flds.png",dpi=150)
plt.show()
