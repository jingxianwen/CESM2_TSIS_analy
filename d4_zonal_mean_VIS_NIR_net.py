...
for iy in range(0,years.size): 
    ...
    for vn in varnms_nir_dn:
        dtctl_dn=file_ctl.variables[vn][0,:,:]
        dtexp_dn=file_exp.variables[vn][0,:,:] 
        means_yby_ctl_dn[iy,1,:]= means_yby_ctl_dn[iy,1,:] + np.mean(dtctl_dn[:,:],axis=1) #[0,:]
        means_yby_exp_dn[iy,1,:]= means_yby_exp_dn[iy,1,:] + np.mean(dtexp_dn[:,:],axis=1) #[0,:]
    ...
    for vn in varnms_nir_up:
        dtctl_up=file_ctl.variables[vn][0,:,:]
        dtexp_up=file_exp.variables[vn][0,:,:] 
        means_yby_ctl_up[iy,1,:]= means_yby_ctl_up[iy,1,:] + np.mean(dtctl_up[:,:],axis=1) #[0,:]
